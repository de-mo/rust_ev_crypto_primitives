use criterion::{BenchmarkId, Criterion, criterion_group, criterion_main};
use rust_ev_crypto_primitives::{Hexa, Integer, OperationsTrait};

pub fn compare_mut_not_mut_bench(c: &mut Criterion) {
    let mut group = c.benchmark_group("Complex Computation with Integer");
    let p = Integer::from_hexa_string("").unwrap();
    let q = Integer::from_hexa_string("").unwrap();
    let z = Integer::from_hexa_string("").unwrap();
    let exp = Integer::from_hexa_string("").unwrap();
    for i in 0..10 {
        let z_i = Integer::from(&z + Integer::from(i));
        group.bench_with_input(BenchmarkId::new("With mut", i), &i, |b, _i| {
            b.iter(|| {
                let mut res = z_i.clone();
                res.mod_exponentiate_mut(&exp, &p).unwrap();
                res.mod_multiply_mut(&q, &p);
                res.mod_exponentiate_mut(&exp, &p).unwrap();
            })
        });
        group.bench_with_input(BenchmarkId::new("Without mut", i), &i, |b, _i| {
            b.iter(|| {
                z_i.mod_exponentiate(&exp, &p)
                    .unwrap()
                    .mod_multiply(&q, &p)
                    .mod_exponentiate(&exp, &p)
                    .unwrap()
            })
        });
    }
    group.finish();
}

criterion_group!(benches, compare_mut_not_mut_bench);
criterion_main!(benches);
