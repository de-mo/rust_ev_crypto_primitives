use criterion::{criterion_group, criterion_main, Criterion};
use rug::Integer;
use rust_ev_crypto_primitives::{
    elgamal::EncryptionParameters, prepare_fixed_based_optimization,
    zero_knowledge_proofs::verify_schnorr, Hexa, OperationsTrait,
};

fn schnorr_proofs_compare_gmpmee_bench(c: &mut Criterion) {
    let mut group = c.benchmark_group("schnorr_proofs_compare_gmpmee");
    let p = Integer::from_hexa_string("").unwrap();
    let q = Integer::from_hexa_string("").unwrap();
    let g = Integer::from_hexa_string("0x4").unwrap();
    let e = Integer::from_hexa_string(
        "0xFE6B909E2C4EC3C9A25315EAF1E99A50452BBD22E75A69C247147E23669D665F",
    )
    .unwrap();
    let z = Integer::from_hexa_string("").unwrap();
    let y = Integer::from_hexa_string("").unwrap();
    let add_info: Vec<String> = vec!["test-0".to_string(), "test-1".to_string()];
    group.bench_function("rug", |b| {
        b.iter(|| {
            verify_schnorr(
                &EncryptionParameters::from((&p, &q, &g)),
                (&e, &z),
                &y,
                &add_info,
            )
            .unwrap()
        })
    });
    prepare_fixed_based_optimization(&g, &p, 16, p.nb_bits() - 1);
    group.bench_function("gmpmee", |b| {
        b.iter(|| {
            verify_schnorr(
                &EncryptionParameters::from((&p, &q, &g)),
                (&e, &z),
                &y,
                &add_info,
            )
            .unwrap()
        })
    });
    group.finish();
}

fn schnorr_proofs_bench(c: &mut Criterion) {
    let p = Integer::from_hexa_string("").unwrap();
    let q = Integer::from_hexa_string("").unwrap();
    let g = Integer::from_hexa_string("0x4").unwrap();
    let e = Integer::from_hexa_string(
        "0xFE6B909E2C4EC3C9A25315EAF1E99A50452BBD22E75A69C247147E23669D665F",
    )
    .unwrap();
    let z = Integer::from_hexa_string("").unwrap();
    let y = Integer::from_hexa_string("").unwrap();
    let add_info: Vec<String> = vec!["test-0".to_string(), "test-1".to_string()];
    prepare_fixed_based_optimization(&g, &p, 16, p.nb_bits() - 1);
    c.bench_function("schnorr_proofs", |b| {
        b.iter(|| {
            verify_schnorr(
                &EncryptionParameters::from((&p, &q, &g)),
                (&e, &z),
                &y,
                &add_info,
            )
            .unwrap()
        })
    });
}

criterion_group!(
    benches,
    schnorr_proofs_compare_gmpmee_bench,
    schnorr_proofs_bench
);
criterion_main!(benches);
