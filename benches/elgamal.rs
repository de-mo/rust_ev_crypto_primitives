...
use rust_ev_crypto_primitives::{
    DecodeTrait,
    elgamal::{EncryptionParameters, combine_public_keys},
};
...
pub fn combine_public_keys_bench(c: &mut Criterion) {
    ...
    let pks = PKS_BASE64
        .iter()
        .map(|&s_slice| Integer::base_64_decode_vector(s_slice.to_vec().as_slice()).unwrap())
        .collect::<Vec<_>>();
    c.bench_function("combine_public_keys", |b| {
        b.iter(|| combine_public_keys(&p, pks.as_slice()).unwrap())
    });
}

pub fn get_small_prime_group_members_bench(c: &mut Criterion) {
    let ep = encryption_parameters();
    c.bench_function("get_small_prime_group_members_bench", |b| {
        b.iter(|| ep.get_small_prime_group_members(5000))
    });
}

criterion_group!(
    benches,
    combine_public_keys_bench,
    get_small_prime_group_members_bench
);
