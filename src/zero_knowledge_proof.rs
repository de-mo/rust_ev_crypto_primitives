use super::{
    hashing::HashableMessage, num_bigint::Operations,
    elgamal::{check_p, ElgamalError, check_encryption_parameters, check_q}, 
    number_theory::{NumberTheoryError, check_quadratic_residue}
};
...
fn compute_phi_schnorr((p, _, g): (&BigUint, &BigUint, &BigUint), x: &BigUint) -> BigUint {
    g.mod_exponentiate(x, p)
}

/// Verify Schnorr Proof according to specifications
pub fn verify_schnorr(
    (p, q, g): (&BigUint, &BigUint, &BigUint),
    (e, z): (&BigUint, &BigUint),
    y: &BigUint,
    i_aux: &Vec<String>,
) -> Result<bool, ZeroKnowledgeProofError> {
    if cfg!(feature="checks"){
        if let Some(e) = check_encryption_parameters((p,q,g)) {
            return Err(ZeroKnowledgeProofError::CheckElgamal(e))
        }
        if let Some(e) = check_quadratic_residue(y, p) {
    ...
    let x = compute_phi_schnorr((p, q, g), z);
    let f = HashableMessage::from(vec![p, q, g]);
    // e in Z_q => modulo q
    // x, y in G_q => modulo p
    let c_prime = x.mod_multiply(&y.mod_exponentiate(e, p).mod_inverse(p), p);
...
fn compute_phi_exponentiation(
    ...
    gs.iter().map(|g| g.mod_exponentiate(x, p)).collect()
}
...
pub fn verify_exponentiation(
    ...
    if cfg!(feature="checks"){
        ...
        if gs.len() != ys.len() {
            return Err(ZeroKnowledgeProofError::CheckListSameSize("gs".to_string(), "ys".to_string()))
        }
        for g in gs.iter() {
            if let Some(e) = check_quadratic_residue(g, p) {
                return Err(ZeroKnowledgeProofError::CheckNumberTheory(e))
            }
        }
        for y in ys.iter() {
    ...
    let xs = compute_phi_exponentiation((p, q), z, gs);
    let f_list = vec![
...
mod test {
    ...
    fn test_verify_schnorr_check_wrong() {
        let p= BigUint::from(12u8);
        let q=  BigUint::from_hexa_string("").unwrap();
        let g = BigUint::from_hexa_string("0x4").unwrap();
        let e = BigUint::from_hexa_string(
            "0x29D551590009AC768A674AC0C42416FDDCF261DE93CA78ED53632929169406E1",
        )
        .unwrap();
        let z = BigUint::from_hexa_string("").unwrap();
        ...
        if cfg!(feature="checks") {
            assert!(verify_schnorr((&p, &q, &g), (&e, &z), &y, &add_info).is_err())
        } else {
            assert!(verify_schnorr((&p, &q, &g), (&e, &z), &y, &add_info).is_ok())
        }
    }

    #[test]
    fn test_verify_schnorr_2048() {
        let p= BigUint::from_hexa_string("").unwrap();
        let q=  BigUint::from_hexa_string("").unwrap();
        let g = BigUint::from_hexa_string("0x4").unwrap();
        let e = BigUint::from_hexa_string(
            "0x29D551590009AC768A674AC0C42416FDDCF261DE93CA78ED53632929169406E1",
        ...
        let y = BigUint::from_hexa_string("").unwrap();
        let add_info: Vec<String> = vec!["test-0".to_string()];
        assert!(verify_schnorr((&p, &q, &g), (&e, &z), &y, &add_info).unwrap())
    }

    #[test]
    fn test_verify_schnorr_3072() {
        let p = BigUint::from_hexa_string("").unwrap();
        let q = BigUint::from_hexa_string("").unwrap();
        let g = BigUint::from_hexa_string("0x4").unwrap();
        let e = BigUint::from_hexa_string(
            "0xFE6B909E2C4EC3C9A25315EAF1E99A50452BBD22E75A69C247147E23669D665F",
        )
        .unwrap();
        let z = BigUint::from_hexa_string("").unwrap();
    ...
    fn test_verify_exp_2048() {
        let p= BigUint::from_hexa_string("").unwrap();
        let q=  BigUint::from_hexa_string("").unwrap();
        let _g = BigUint::from_hexa_string("0x4").unwrap();
        let bases = [
        ...
        let ys: Vec<BigUint> = statement
            ...
            .collect();
        let e = BigUint::from_hexa_string(
            "0x9963C404F260027B5B9C6A7B79F7B3E7A30BFB369EF1E5339A217C88355A9010",
        )
        .unwrap();
        let z = BigUint::from_hexa_string("").unwrap();
        let add_info: Vec<String> = vec!["test-0".to_string()];
    ...
    fn test_verify_exp_2048_2() {
        ...
        let bases = [
            ...
            "",
        ];
        let gs: Vec<BigUint> = bases
            .iter()
            .map(|e| BigUint::from_hexa_slice(e).unwrap())
        ...
        let ys: Vec<BigUint> = statement
            ...
            .collect();
        let e = BigUint::from_hexa_string(
            "0x394374B59A3ECAD645C9B23A9614923F97116F069B6101B7CEEAFD774CC5E172",
        )
        .unwrap();
        let z = BigUint::from_hexa_string("").unwrap();
        let add_info: Vec<String> = vec!["test-0".to_string()];
        assert!(verify_exponentiation(
            (&p, &q),
            &gs,
    ...
    fn test_verify_exp_2048_7() {
        ...
        let bases = [
            ...
            "",
        ...
        let ys: Vec<BigUint> = statement
            ...
            .collect();
        let e = BigUint::from_hexa_string(
            "0xC16A0A29503089F591FAF4C32C9DE200578526A3476AA42A13AC5B6508D75679",
        )
        .unwrap();
        let z = BigUint::from_hexa_string("").unwrap();
    ...
    fn test_verify_exp_3072() {
        ...
        .unwrap();
        let z = BigUint::from_hexa_string("").unwrap();
        let add_info: Vec<String> = vec!["test-0".to_string()];
        assert!(verify_exponentiation(
            (&p, &q),
            &gs,
    ...
    fn test_verify_exp_3072_2() {
        ...
        .unwrap();
        let z = BigUint::from_hexa_string("").unwrap();
        let add_info: Vec<String> = vec!["test-0".to_string(), "test-1".to_string()];
        assert!(verify_exponentiation(
            (&p, &q),
            &gs,
    ...
    fn test_verify_exp_3072_7() {
        ...
        .unwrap();
        let z = BigUint::from_hexa_string("").unwrap();
        let add_info: Vec<String> = vec!["test-0".to_string(), "test-1".to_string()];
        assert!(verify_exponentiation(
            (&p, &q),
            &gs,
