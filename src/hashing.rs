...
use crate::{
    ...
    integer::MPInteger,
    MPIntegerError, GROUP_PARAMETER_Q_LENGTH, SECURITY_STRENGTH,
};
...
pub trait RecursiveHashTrait {
    ...
    ///
    /// Return [Self::Error] if an error appears during the calculation
    fn recursive_hash_to_zq(&self, q: &MPInteger) -> Result<MPInteger, Self::Error>;
}
...
pub enum HashError {
    ...
    #[error(transparent)]
    MPIntegerError(#[from] MPIntegerError),
    ...
    fn to_hashable_byte_array(&self) -> Result<ByteArray, HashError> {
        match self {
            ...
            HashableMessage::RInt(i) => Ok(ByteArray::try_from(*i)
                .map_err(HashError::MPIntegerError)?
                .prepend_byte(1u8)),
