...
/// # Error
/// [OpensslError] if something is going wrong
pub fn shake256(byte_array: &ByteArray) -> Result<ByteArray, BasisCryptoError> {
    let mut digest = [0; GROUP_PARAMETER_P_LENGTH / 8];
    hash_xof(
...
mod test {
    ...
    fn test_shake128() {
        assert_eq!(
            shake128(&ByteArray::from_bytes(b"Some Crypto Text")).unwrap(),
    ...
    fn test_shake256() {
        assert_eq!(
            shake256(&ByteArray::from_bytes(b"Some Crypto Text")).unwrap(),
            ByteArray::base16_decode(
