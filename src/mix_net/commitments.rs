...
pub enum CommitmentError {
    ...
    #[error(transparent)] MatrixError(#[from] MatrixError),
    #[error("Matrix is mallformed in: {0}")] MalformedMatrix(String),
    #[error("Size {0} of random vector must be {1}")] RandomSizeWrong(usize, usize),
...
impl CommitmentKey {
        ...
        while count <= nu {
            let u =
                HashableMessage::from(
                    ...
                    .map_err(CommitmentError::HashError)? + MPInteger::one();
            let w = u.mod_square(ep.p());
            if &w != MPInteger::one() && &w != ep.g() && !v.contains(&w) {
                v.push(w);
                count += 1;
            }
            i += 1;
    ...
    pub fn to_vec(&self) -> Vec<MPInteger> {
        let mut res: Vec<MPInteger> = self.gs.clone();
...
pub fn get_commitment(
    ...
    r: &MPInteger,
    ck: &CommitmentKey
) -> Result<MPInteger, CommitmentError> {
    if ck.gs.len() < a.len() {
    ...
    let prod: rug::Integer = a
        ...
        .fold(MPInteger::one().clone(), |acc, v| acc * v);
...
pub fn get_commitment_matrix(
    ...
    rs: &[MPInteger],
    ck: &CommitmentKey
) -> Result<Vec<MPInteger>, CommitmentError> {
    if a.is_malformed() {
        return Err(CommitmentError::MalformedMatrix("get_commitment_matrix".to_string()));
    }
    ...
    a.columns_cloned_iter()
        ...
        .collect::<Result<Vec<MPInteger>, CommitmentError>>()
}
    ...
    fn test_recursive_hash() {
        let ck = CommitmentKey {
            ...
            gs: [
                ...
                .iter()
                .map(|v| MPInteger::from_hexa_string(v).unwrap())
                .collect(),
        };
        let h = &ck.h;
        let gs_1 = &ck.gs[0];
        let gs_2 = &ck.gs[1];
        let expected = HashableMessage::from(
            vec![HashableMessage::from(h), HashableMessage::from(gs_1), HashableMessage::from(gs_2)]
        )
            .recursive_hash()
            .unwrap();
        assert_eq!(HashableMessage::from(&ck).recursive_hash().unwrap(), expected);
    }
